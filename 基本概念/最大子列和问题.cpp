...
// 方法一：确定子列的首部和尾部，再遍历累加，时间复杂度 T( N ) = O( N3 )
...
// 方法二：确定子列的首部，逐个累加，时间复杂度 T( N ) = O( N2 )
...
// 方法三：分而治之 T ( N ) = 2 T( N/2 ) + c N , T(1) = O(1)
//= 2 [2 T( N/22 ) + c N/2] + c N
//= 2k O(1) + c k N 其中 N/2k = 1
//= O( N log N )
...
int main() {
    ...
    for (int i = 0; i < n; i++)
        cin >> a[i];
    int s1 = MaxSubseqSum1(a, n);
    int s2 = MaxSubseqSum2(a, n);
    int s3 = MaxSubseqSum3(a, n);
    int s4 = MaxSubseqSum4(a, n);
